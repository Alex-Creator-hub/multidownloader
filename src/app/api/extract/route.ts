...
async function scrapeTweetPage(tweetId: string): Promise<Media[] | null> {
  ...
  try {
    ...
    let tweet: any = null;
    const tr = pageProps?.tweetResult;
    if (tr?.__typename === "Tweet") tweet = tr;
    if (!tweet) {
      for (const entry of pageProps?.conversationThread?.instructions?.[0]?.entries || []) {
    ...
    return rawMedia.map((m: any) => {
      if (m.type === "video" || m.type === "animated_gif") {
        ...
        return { type: "video" as const, title: text, previewUrl: m.media_url_https || "", formats: variants.map((v: any) => ({ quality: labelForBitrate(v.bitrate), url: v.url })) };
      }
      return { type: "image" as const, title: text, previewUrl: m.media_url_https || "", formats: [{ quality: "原图", url: m.media_url_https + "?name=orig" }] };
    });
  } catch { return null; }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!mediaList || mediaList.length === 0)
      return NextResponse.json({ error: "该推文中没有检测到视频或图片，推文可能不存在或为私密账号" }, { status: 400 });
